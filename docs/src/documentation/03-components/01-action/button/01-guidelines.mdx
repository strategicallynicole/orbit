...
- When you have many actions at once---use a [button link](/components/buttonlink/).
- You want to offer a less important action---use a [button link](/components/buttonlink/).
- To make text inside paragraphs or lists actionable---use a [text link](/components/textlink/).
- So users can sign in using a social service---use a [SocialButton](/components/socialbutton/).
...
you can indicate importance through [size, type, and variation](#button-combinations).

<Guidelines type="do" title="Have one primary action">
...
<Guidelines type="do" title="Show importance hierarchy">
...
But such buttons often end up confusing users about what is possible.

Instead, try to focus on using [progressive disclosure](/guides/progressive-disclosure/)
to show them only the actions they should take right at the moment.
Then when it is possible for them to take action, you can show them more buttons.

If you need to use a disabled button,
always make it clear how users can activate it,
such as with a [tooltip](/components/tooltip/).
...
See [examples for how to make actions clear](/content/voice-tone/straightforward/#make-every-action-clear).
...
to show actions that won't draw as much attention.
...
<ReactExample name="Button types" />

<Guidelines type="do" title="Use secondary small buttons for additional actions on the page">


The secondary small button is great for actions in [cards](/components/cards/)
as it stands out from the interface but doesn't take so much attention.

</Guidelines>


<Guidelines type="do" title="Use secondary buttons for items in loop">
...
<Guidelines type="do" title="Don't use primary buttons to manipulate users">
...
<ReactExample name="Buttons with icons" />

<Guidelines type="do" title="Use a left icon when the button adds another item">
...
<Guidelines type="do" title="Right icons should only be directional">


They can help explain what happens when the user interacts with the button.
It's easier to see if the user will be taken elsewhere with an icon.

</Guidelines>


<Guidelines type="do" title="Add a label when using icon-only buttons">
...
![](../../../../images/components/actions/button-interactions.png)
