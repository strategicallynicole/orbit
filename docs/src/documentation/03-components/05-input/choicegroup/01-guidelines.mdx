...
<FigmaFile
  ...
  node="0%3A216"
  alt="Label: presents a clear opening statement to help users pick; choices: choices to pick from -- usually Radio or Checkbox, depending on if users should pick one or more options; error message: helps users through the problem to a solution."
/>
...
For many options, consider using a [select](/components/select/).
...
Follow the labeling advice for [radios](/components/radio/#use-parallel-labels)
or [checkboxes](/components/checkbox/#write-positive-labels).
...
<ReactExample name="ChoiceGroup error" />
